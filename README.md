...
Tessera is live at <https://stephenfreelance.github.io/tiles-generator/>, published to GitHub Pages from `main`.
...
- **Base path.** A project site lives under `/tiles-generator/`, so the `pages` job builds with `npm run build -- --base "/tiles-generator/"` (the path comes from `actions/configure-pages` and would be empty on a custom domain). Vite rewrites the URLs in `index.html` and the CSS, and the router takes its `basename` from `import.meta.env.BASE_URL`. Everywhere else the base stays `/`. To try the Pages build locally, run `npm run build -- --base /tiles-generator/ && npx vite preview --base /tiles-generator/` and open <http://localhost:4173/tiles-generator/>.
...
- **Crawlable assets.** The page is client-rendered, so `public/robots.txt` deliberately allows `/assets/`. Blocking it leaves a crawler with an empty page. Crawlers only read `robots.txt` at the root of a host, so on the project site it has no effect.

`index.html` carries the share metadata. The canonical URL is written from the address the page is served at, and `og:url` and `og:image` are relative and follow the build's base, so both are correct at whatever origin this ships on; make them absolute once there is a domain. Regenerate the share card with `npm run shots -- --og` after a visual change.
...
Tests run in vitest's node environment and cover `src/core`, `src/workers` and `src/state`: the pure logic and the data boundaries. Component behaviour is checked in a real browser instead of jsdom, through the capture harness, which defaults to port 5184:
